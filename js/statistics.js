...
function updateGroupsOverview() {
    ...
    sortedGroups.forEach(group => {
        ...
        // Create detail items
        let details = '';
        details += `<span class="group-stat-detail">${groupPersons.length} Members</span>`;
        if (elderCount > 0) details += `<span class="group-stat-detail">${elderCount} Elders</span>`;
        if (servantCount > 0) details += `<span class="group-stat-detail">${servantCount} Servants</span>`;
        if (pioneerCount > 0) details += `<span class="group-stat-detail">${pioneerCount} Pioneers</span>`;
        if (leaderCount > 0) details += `<span class="group-stat-detail">${leaderCount} Leaders</span>`;
        if (helperCount > 0) details += `<span class="group-stat-detail">${helperCount} Helpers</span>`;
        if (groupMeetings.length > 0) details += `<span class="group-stat-detail">${groupMeetings.length} Meeting Points</span>`;
        ...
        item.querySelector('.view-group-btn').addEventListener('click', () => {
